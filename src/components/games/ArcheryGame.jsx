...
export default function ArcheryGame({
  ...
  useEffect(() => {
    ...
    class ArcheryScene extends Phaser.Scene {
      ...
      createGoldenTargetTimer() {
        this.time.addEvent({
          delay: 17000,
          loop: true,
          callback: () => {
            if (!ready || ended || goldenTarget?.active) return;
            this.spawnGoldenTarget();
          },
        });
      }

      spawnGoldenTarget() {
        goldenTarget = this.add.container(
          Phaser.Math.Between(GAME_W - 480, GAME_W - 150),
          Phaser.Math.Between(110, GAME_H - 190)
        ).setDepth(85);

        goldenTarget.add(this.add.circle(0, 0, 42, 0xfacc15, 0.95));
        goldenTarget.add(this.add.circle(0, 0, 29, 0xffffff, 1));
        goldenTarget.add(this.add.circle(0, 0, 17, 0xfacc15, 1));
        goldenTarget.add(this.add.text(0, -62, "⭐", { fontSize: "30px" }).setOrigin(0.5));
        goldenTarget.baseY = goldenTarget.y;
        goldenTarget.movePhase = Phaser.Math.FloatBetween(0, Math.PI * 2);

        this.physics.add.existing(goldenTarget);
        goldenTarget.body.setCircle(44);
        goldenTarget.body.setAllowGravity(false);
        this.centerMessage("⭐ Golden Target appeared!", "#facc15");
        ...
        this.time.delayedCall(7000, () => {
          if (goldenTarget?.active) {
            goldenTarget.destroy();
            goldenTarget = null;
          }
        });
      }
      ...
      startDrag(pointer) {
        ...
        if (distanceFromBow > 180 && !isPortraitMobile) return;
      ...
      getShotData(pointer = null) {
        ...
        const maxDrag = isPortraitMobile ? 190 : 230;
      ...
      releaseArrow() {
        ...
        this.arrow.body.setVelocity(velocity.x, velocity.y);
        this.arrow.rotation = Math.atan2(velocity.y, velocity.x);
      ...
      moveTargets(time) {
        ...
        if (goldenTarget?.active) {
          goldenTarget.y = goldenTarget.baseY + Math.sin(time / 420 + goldenTarget.movePhase) * 70;
        }
      }
      ...
      checkTargetHits() {
        ...
        this.targets.forEach((target) => {
          ...
          if (d < (target.isBoss ? 72 : 56)) {
            target.correct ? this.hitCorrect(target) : this.hitWrong(target);
          }
        });
      }

      checkGoldenHit() {
        if (!goldenTarget?.active || !this.arrow?.visible || !this.arrow.body?.enable) return;
        const d = Phaser.Math.Distance.Between(this.arrow.x, this.arrow.y, goldenTarget.x, goldenTarget.y);
        if (d < 56) {
          score += 25;
          combo += 1;
          this.centerMessage("⭐ GOLDEN HIT +25", "#facc15");
          this.floatText(goldenTarget.x, goldenTarget.y - 55, "+25", "#facc15");
          goldenTarget.destroy();
          goldenTarget = null;
          this.resetArrow();
          syncHud("Golden target collected. Continue question.");
        }
      }
      ...
      hitCorrect(target) {
        ...
        const gained = Math.round((15 + combo * 4 + (target.isBoss ? 35 : 0)) * boost);
        score += gained;
        combo += 1;
        this.centerMessage(target.isBoss ? "🦅 BOSS DEFEATED!" : combo >= 3 ? `COMBO x${combo}!` : "CORRECT HIT!", target.isBoss ? "#facc15" : "#22c55e");
  ...
  }, [mapSelected, selectedMapIndex, questions, topic, isPortraitMobile]);
  ...
  const HudCard = ({ mobile = false }) => (
    ...
    >
      ...
      <div className="mt-3 text-xs font-black text-yellow-200">{hud.mapName}</div>
  ...
  if (isPortraitMobile) {
    return (
      ...
      >
        ...
        <div className="relative border-b border-yellow-400/30 bg-slate-950 px-3 pt-5">
          <HudCard mobile />
        </div>
