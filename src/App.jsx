...
/**
 * FishGame V6.5 - Portrait Reels Layout
...
export default function FishGame({
  ...
  useEffect(() => {
    ...
    // Portrait mobile reels layout:
    // top HUD panel, middle horizontal 16:9 ocean strip, bottom joystick panel.
    const SAFE_TOP = isMobileDevice ? Math.max(62, GAME_H * 0.085) : 0;
    const SAFE_BOTTOM = isMobileDevice ? Math.max(34, GAME_H * 0.055) : 0;

    const PORTRAIT_HUD_H = isPortraitMobile ? Math.max(150, GAME_H * 0.24) : 0;
    const PORTRAIT_CONTROL_H = isPortraitMobile ? Math.max(185, GAME_H * 0.27) : 0;

    const PORTRAIT_GAME_H = isPortraitMobile
      ? Math.min(GAME_W * 9 / 16, GAME_H - PORTRAIT_HUD_H - PORTRAIT_CONTROL_H)
      : GAME_H;

    const PLAY_TOP = isPortraitMobile
      ? PORTRAIT_HUD_H + Math.max(0, (GAME_H - PORTRAIT_HUD_H - PORTRAIT_CONTROL_H - PORTRAIT_GAME_H) / 2)
      : 0;

    const PLAY_BOTTOM = isPortraitMobile ? PLAY_TOP + PORTRAIT_GAME_H : GAME_H;
    ...
    class OceanScene extends Phaser.Scene {
      constructor() {
        ...
        this.joyCenterY = isPortraitMobile
          ? PLAY_BOTTOM + (GAME_H - PLAY_BOTTOM - SAFE_BOTTOM) * 0.52
      ...
      create() {
        ...
        // Do not change camera viewport in portrait.
        // Instead, fixed top/bottom panels cover the world,
        // so the visible ocean appears as a horizontal center strip.
        ...
        this.time.delayedCall(1000, () => {
          ...
          this.setMessage(isMobileDevice ? "Use joystick below. Ocean stays horizontal like laptop." : "Move with mouse/touch. Chase ⭐ answer fish.");
        });
      ...
      createFixedHud() {
        ...
        if (isPortraitMobile) {
          // Opaque top panel, like mobile reels/game HUD.
          const topBg = this.add.rectangle(GAME_W / 2, PORTRAIT_HUD_H / 2, GAME_W, PORTRAIT_HUD_H, 0x020617, 0.96);
          hud.add(topBg);

          this.questionText = this.add.text(16, SAFE_TOP - 34, "", {
            fontSize: Math.max(13, GAME_W * 0.04) + "px",
            color: "#ffffff",
            fontFamily: "Arial",
            fontStyle: "bold",
            wordWrap: { width: GAME_W - 112 },
            stroke: "#000000",
            strokeThickness: 5,
          });
          hud.add(this.questionText);

          this.exitButton = this.add.text(GAME_W - 54, SAFE_TOP - 18, "EXIT", {
          ...
          const pillStyle = {
            fontSize: Math.max(11, GAME_W * 0.03) + "px",
            color: "#ffffff",
            fontFamily: "Arial",
            fontStyle: "bold",
          };

          const rowY = SAFE_TOP + 8;
          this.hudScore = this.add.text(16, rowY, "Score: 0", pillStyle);
          this.hudHealth = this.add.text(GAME_W * 0.26, rowY, "HP: 100", { ...pillStyle, color: "#86efac" });
          this.hudCombo = this.add.text(GAME_W * 0.50, rowY, "Combo: 0", { ...pillStyle, color: "#93c5fd" });
          this.hudLevel = this.add.text(GAME_W * 0.76, rowY, "Lv: 1", { ...pillStyle, color: "#c4b5fd" });

          hud.add([this.hudScore, this.hudHealth, this.hudCombo, this.hudLevel]);

          hud.add(this.add.rectangle(GAME_W / 2, rowY + 34, GAME_W - 32, 14, 0x0f172a, 0.95));
          this.healthBar = this.add.rectangle(16, rowY + 34, GAME_W - 32, 9, 0x22c55e, 1).setOrigin(0, 0.5);
          hud.add(this.healthBar);

          this.messageText = this.add.text(16, rowY + 52, "Preparing ocean...", {
            fontSize: Math.max(10, GAME_W * 0.028) + "px",
            color: "#cbd5e1",
            fontFamily: "Arial",
            fontStyle: "bold",
          ...
          // Top/bottom masks make the ocean appear as a horizontal strip.
          hud.add(this.add.rectangle(GAME_W / 2, PLAY_TOP - 2, GAME_W, 4, 0x38bdf8, 0.35));
          hud.add(this.add.rectangle(GAME_W / 2, PLAY_BOTTOM + 2, GAME_W, 4, 0x38bdf8, 0.35));

          // Bottom joystick panel.
          const bottomH = GAME_H - PLAY_BOTTOM;
          const bottomBg = this.add.rectangle(GAME_W / 2, PLAY_BOTTOM + bottomH / 2, GAME_W, bottomH, 0x020617, 0.96);
          hud.add(bottomBg);

          const hint = this.add.text(GAME_W / 2, PLAY_BOTTOM + 18, "Joystick area • Chase ⭐ answer fish", {
            fontSize: Math.max(10, GAME_W * 0.028) + "px",
            color: "#bae6fd",
            fontFamily: "Arial",
            fontStyle: "bold",
          ...
          this.pointerStar = this.add.text(GAME_W / 2, PLAY_TOP + 28, "⭐", {
            fontSize: "28px",
      ...
      createMiniMap() {
        if (isPortraitMobile) {
          this.map = this.add.container(GAME_W - 70, SAFE_TOP + 88).setScrollFactor(0).setDepth(520);

          const bg = this.add.rectangle(0, 0, 112, 68, 0x020617, 0.80);
          bg.setStrokeStyle(2, 0x38bdf8, 0.55);

          const title = this.add.text(-48, -30, "MAP", {
            fontSize: "10px",
            color: "#93c5fd",
            fontFamily: "Arial",
            fontStyle: "bold",
      ...
      updateMiniMap() {
        ...
        const mapW = isPortraitMobile ? 112 : (isMobileDevice ? 230 : 310);
        const mapH = isPortraitMobile ? 68 : (isMobileDevice ? 132 : 178);
      ...
      endGame(reason = "Game complete!") {
        ...
        if (!rewardSentRef.current && typeof rewardRef.current === "function") {
          ...
          rewardRef.current({
            ...
            mode: "fish-ocean-v6-5-portrait-reels-layout",
          });
        }
      }
    }
